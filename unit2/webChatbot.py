...
def get_iss_position() -> dict:
    """
    Tool call function that retrieves the current position of the ISS.
    ...
    if data.get("message") != "success":
        raise ValueError(f"API returned non-success message: {data.get('message')}")
    ...
    return {
        ...
        "timestamp": timestamp,
        "description": f"The ISS is currently at {position['latitude']}° latitude, {position['longitude']}° longitude (as of {timestamp})."
    }
